...
function DashboardShell({ children }) {
  ...
  // Claims any links made anonymously on the landing page.
  //
  // Here rather than on a specific onboarding step: this fires whichever route
  // got someone into the app — signup, an invite, or just signing back in on
  // the browser they used to try the hero. One place instead of three.
  //
  // Safe to run on every mount. The endpoint scopes its update to links still
  // in the public workspace and clears the cookie on success, so a second call
  // finds nothing to do.
  useEffect(() => {
    // The cookie is httpOnly, so this can't check whether one exists — the
    // endpoint answers that. It's a single request that usually returns
    // { claimed: 0 } and costs nothing.
    let cancelled = false
    fetch('/api/public/claim', { method: 'POST' })
      ...
      .catch(() => {
        // A failed claim isn't worth telling anyone about — the cookie
        // survives, so it retries on the next visit.
      })
    return () => {
      cancelled = true
    }
