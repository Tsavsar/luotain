...
// one-path change.
export default function LogoMark({ size = 32, muted = false }) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      ...
      <path
        ...
        // Muted keeps the hook a distinct shade rather than matching
        // the shell — same colour on both paths would flatten the two
        // shapes into one silhouette and the mark stops reading as
        // the mark.
        fill={muted ? 'var(--text-disabled)' : 'var(--primary-base)'}
      />
    </svg>
  )
}
