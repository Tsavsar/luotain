...
// ─── LogoMark ───
// The Luotain hexagon. Was inlined directly in dashboardmenu.jsx;
// pulled out here because the link detail page needs it too, and
// duplicated path data is the kind of thing that silently drifts the
// first time the mark gets adjusted.
//
// `muted` desaturates it for use as a placeholder rather than as
// branding. Note the shell is var(--bg-subtle) even in the normal
// state — only the hook carries the brand colour, so muting is a
// one-path change.
//
// `color` recolours the whole mark into a single hue, for when it sits
// inside something with its own palette — a QR code being the case that
// needed it. The shell drops to 35% of that colour rather than matching
// the hook: at the same value the two shapes merge into one blob and
// the mark stops being recognisable, which is worse than losing the
// two-tone.
export default function LogoMark({ size = 32, muted = false, color }) {
  const shellFill = color ? color : 'var(--bg-subtle)'
  const hookFill = color
    ? color
    : muted
      ? 'var(--text-disabled)'
      : 'var(--primary-base)'
